...
function printRoutes() {
	...
	if (last) {
		...
		hops.forEach(async hop => {
			console.log(hop.addr);
			const who = await whoisLookup(hop.addr)
			if (!whoises[hop.addr]) {
				whoises[hop.addr] = who;
				// console.log('WHOIS', hop.addr);
				// console.log(who);
			}
		})
	}
...
module.exports = async function main(target) {
	var ip = await dnsLookup(target);
